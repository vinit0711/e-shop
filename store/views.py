...
def home(request):
    if request.method=='GET':
        ...
        context = {'products': products , 'categories': categories ,'customer': customer }
        print(" you are in home get  :  " ,request.session.get('email'))
...
def registerUser(request):  # function handling post request of sign up
     #return HttpResponse ("post Request")
        first_name=request.POST.get('firstname')   # corresponds to field "name" in html template
    ...
    # Form Saving
        ...
        if not error_message :
            customer.password=make_password(customer.password)  # before storing in database , hasing of password 
...
def login(request):
    if request.method=='GET':
        ...
        password=request.POST.get('password')
        # requirements
        #1 get customer object by filtering according to the email provided
        customer=Customer.getcustomer(email)  # created function getcustomer
        print(customer)
        error_message =None
        if customer :
            flag =check_password(password ,customer.password)
            if flag :
                request.session['customer']=customer.id  # adding values in session
...
def checkout(request):
    ...
    if request.method=='POST':
        ...
        products=Product.get_all_products_by_id(product)  # this will show all products by ids in cart
